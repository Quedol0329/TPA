...
var is_moving: ScoreboardObjective;
system.run(() => {
  is_moving = world.scoreboard.getObjective("is_moving") || world.scoreboard.addObjective("is_moving", "dummy");
});

var requestList: { [key: string]: Player[] } = {};

function tpa(origin: CustomCommandOrigin, players?: Player[]): CustomCommandResult {
  if (!players) return { status: CustomCommandStatus.Failure, message: "대상을 정해주세요." };
  ...
  var count = system.runInterval(() => {
    ...
    let speed = is_moving.getScore(sender) as number;
    if (speed >= 1) {
...
function tpacancel(origin: CustomCommandOrigin, players?: Player[]): CustomCommandResult {
  if (!players) return { status: CustomCommandStatus.Failure, message: "대상을 정해주세요." };

  const sender = origin.sourceEntity as Player;

  for (let player of players) {
    if (!requestList[player.name].includes(sender)) continue;
...
function tpaaccept(origin: CustomCommandOrigin, players?: Player[]): CustomCommandResult {
  if (!players) return { status: CustomCommandStatus.Failure, message: "대상을 정해주세요." };

  const sender = origin.sourceEntity as Player;

  for (let player of players) {
    if (!requestList[sender.name].includes(player)) continue;
...
function tpadeny(origin: CustomCommandOrigin, players?: Player[]): CustomCommandResult {
  if (!players) return { status: CustomCommandStatus.Failure, message: "대상을 정해주세요." };
  const sender = origin.sourceEntity as Player;
  for (let player of players) {
    if (!requestList[sender.name].includes(player)) continue;
...
world.afterEvents.playerJoin.subscribe((data: PlayerJoinAfterEvent) => {
